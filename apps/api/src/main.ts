...
async function bootstrap() {
  ...
  expressApp.use((req: Request, res: Response, next: NextFunction) => {
    // Skip timeout for health checks, liveness probe, and warmup (needed for Docker/Traefik)
    if (req.path === '/api/ping' || req.path === '/api/health' || req.path === '/api/health/deep' || req.path === '/api/warmup') {
      return next();
    }
