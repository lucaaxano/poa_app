...
export class AuthService {
  ...
  async validateUser(userId: string): Promise<(User & { company: Company | null }) | null> {
    // NOTE: Do NOT add ensureConnectionReady() here - this method is called on EVERY
    // authenticated request and would cause massive slowdown
    const startTime = Date.now();
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      include: { company: true },
  ...
  async getProfile(userId: string): Promise<ProfileResponse> {
    // NOTE: Do NOT add ensureConnectionReady() here - getProfile is called frequently
    // The 10s keep-alive and login pre-warming are sufficient
    const startTime = Date.now();
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      include: { company: true },
